...
const Dashboard=()=>{
  ...
  useEffect(()=>{ if(isEdit&&product?._id===id){ const { productNameOption, customProductName }=getProductNameFields(product.name); setForm({name:product.name||'',productNameOption,customProductName,price:product.price??'',brand:product.brand||'',category:product.category||'',countInStock:product.countInStock??'',description:product.description||'',codAvailable:product.codAvailable!==false,features:product.features?.length?product.features:[createFeature()],specifications:product.specifications?.length?product.specifications:[createSpec()]}); } },[isEdit,product,id]);
  ...
  const updateFeature=(index,value)=>setForm(prev=>({ ...prev, features: prev.features.map((feature,i)=>i===index?value:feature) }));
  ...
  const onSubmit=async(e)=>{
    ...
    fd.append('features',JSON.stringify(form.features.map(feature=>feature.trim()).filter(Boolean)));
    fd.append('specifications',JSON.stringify(form.specifications.filter(spec=>spec.name.trim()&&spec.value.trim())));
    ...
    try{
      ...
      setImages([]); setForm(emptyForm); navigate('/admin/manage-products');

...
const getSpecificationFields=(name='',value='',category='',specificationOptionsMap=DEFAULT_SPECIFICATION_OPTIONS_BY_CATEGORY)=>{
  ...
  if(!normalizedName) return { option:'', customName:'', value };
  return options.includes(normalizedName)
    ? { option:normalizedName, customName:'', value }
    : { option:OTHER_SPECIFICATION_OPTION, customName:normalizedName, value };
};
...
const createSpec=()=>({ option:'', customName:'', value:'' });
...
const getFeatureFields=(feature='')=>{
  ...
  if(!normalizedFeature) return { option:'', customValue:'' };
  return FEATURE_OPTIONS.includes(normalizedFeature)
    ? { option:normalizedFeature, customValue:'' }
    : { option:OTHER_FEATURE_OPTION, customValue:normalizedFeature };
};
...
const createFeature=()=>({ option:'', customValue:'' });
...
const Dashboard=()=>{
  ...
  const onAddFeatureOption=(index)=>{
    ...
    setForm(prev=>({ ...prev, features: prev.features.map((feature,i)=>i===index?{ option:normalizedFeature, customValue:'' }:feature) }));
  };
  ...
  const onDeleteFeatureOption=(index)=>{
    ...
    setForm(prev=>({ ...prev, features: prev.features.map((feature,i)=>feature.option===selectedOption?{ option:i===index?'':feature.option, customValue:'' }:feature) }));
  };
